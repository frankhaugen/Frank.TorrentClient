...
public class PieceManagerTest
{
    ...
    public void TestPieceManager()
    {
        ...
        string[] pieceHashes;
        byte[] data;
        var blockData = new byte[2];
        var pieceData = new byte[4];
        Piece piece;
        ...
        pieceManager = new PieceManager(data.CalculateSha1Hash().ToHexaDecimalString(), data.Length, pieceHashes, 4, 2,
            bitField);
        pieceManager.PieceCompleted += (sender, e) =>
        {
            ...
            if (pieceManager.IsComplete)
                CollectionAssert.AreEqual(
                    new[] { PieceStatus.Present, PieceStatus.Present, PieceStatus.Present, PieceStatus.Present },
                    pieceManager.BitField);
        };

        CollectionAssert.AreEqual(
            new[] { PieceStatus.Missing, PieceStatus.Missing, PieceStatus.Missing, PieceStatus.Missing },
            pieceManager.BitField);
        ...
        foreach (var pieceIndex in new[] { 0, 3, 2, 1 })
        {
            ...
            foreach (var blockOffseet in new[] { 0, 2 })
            {
